...
## Super Challenge Task: Blur

This is a very difficult challenge, but if you want to try it, you can attempt to blur your image. Blurring an image is a bit like smudging, but you take the average of the colors of neighboring pixels and apply that average to the current pixel. This can be done in a similar way to the smudge, however, if you want to create a true blur, you will need to use a second 2D array to store the new pixel values. This is because you will need to use the original pixel values to calculate the new pixel values, and you can't do that if you are altering the original pixel values as you go. Of course, you will also need a new set of functions, similar to the smudge, as existing functions will not be able to handle the complexity of a blur.
